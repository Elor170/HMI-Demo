...
export const getWaterfallData = async (): Promise<WaterfallObject[]> => {
  const response = await axios.get(WATERFALL_BACKEND_URL + '/older-waterfall-data', {
    params: {
        time: new Date(),
    }
  });

  return response.data.reverse() as unknown as WaterfallObject[];
}


export const initCanvas = (canvas: HTMLCanvasElement | null, waterfallData: WaterfallObject[]) => {
  if (canvas) {
