...
const constG: readonly number[] = new Array(arraysLength)
  .fill(0)
  .map((_, i) => {
    ...
    return Math.floor(
      (GMaxVal * (distanceFromEdge - (distanceFromEdge % constCubeWidth))) /
        middleIndex,
    );
  });
...
const createRandomG = (): readonly number[] => {
  ...
  if (randomLinesCounter === 10) {
    ...
    randomGArr = result;
  } else randomLinesCounter += 1;

  return randomGArr;
};

export default function generateData(
  sendingInt: SendingInterval,
): WaterfallObject {
  let G;
  const R = zerosArr;
  const B = zerosArr;
