...
export const saveMsg = async (newData: WaterfallObject): Promise<void> => {
  if (DB) {
    try {
      ...
      if (!DB.getIsWhileConnecting()) DB.reconnect();
    }
  }
};
...
export const updateMsg = async (newData: WaterfallObject): Promise<void> => {
  if (DB) {
    try {
      await DB.updateOne(
        "waterfall",
        { sendingTime: newData.sendingTime },
        newData,
      );
    } catch (error) {
...
export const buildDataFrame = async (
  dataArr: WaterfallObject[],
): Promise<WaterfallDataFrame> => {
  try {
...
export const saveMultipleMsgs = async (
  newDataArr: WaterfallObject[],
): Promise<void> => {
  if (DB) {
    try {
