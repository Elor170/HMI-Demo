...
export const addGrayLines = async (): Promise<void> => {
  ...
  await saveMultipleMsgs(
    Array.from({ length: 10 }, () => structuredClone(grayLine)),
  );
};

const onLostConnection = async (): Promise<void> => {
  addGrayLines();
  reconnectDataReceiver().then(async () => receiveMsg(queueName, dataHandler));

  intervalID = setInterval(() => {
    reconnectDataReceiver().then(async () =>
      receiveMsg(queueName, dataHandler),
    );
  }, disconnectTimeout);
};
